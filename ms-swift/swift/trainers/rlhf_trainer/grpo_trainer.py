...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _prepare_batch_inputs(self, inputs: DataType) -> List[DataType]:
        ...
        for batch in gas_chunks:
            ...
            batch_encoded_inputs.update({
                ...
                'truncated_mask': torch.tensor([b['is_truncated'] for b in batch], dtype=torch.bool),
                'logits_to_keep': logits_to_keep,
                'advantages': all_advandages
            })
    ...
    def _get_per_token_logps_and_entropies(self,
        ...
        print("Input IDS shape in logps:", inputs['input_ids'].shape)
    ...
    def _get_per_token_logps_and_entropies_single(self,
        ...
        if can_use_super:
            ...
            if 'logits_to_keep' in self.model_kwarg_keys:
                ...
                # logits_to_keep = logits_to_keep + 1  # to keep the original behavior [update]

            logits = model(**inputs).logits
            # exclude the last logit: it corresponds to the next token pred
            
            print("To Keep Size = ", logits_to_keep)
            print("Logits shape:", logits.shape)
            logits = logits[:, -(logits_to_keep + 1):-1, :]
            print("after keep Logits shape:", logits.shape)

            logits = logits / self.temperature
            
            print("Input ids shape:", input_ids.shape)
            input_ids = input_ids[:, -logits_to_keep:]
            print("after keep Input ids shape:", input_ids.shape)
